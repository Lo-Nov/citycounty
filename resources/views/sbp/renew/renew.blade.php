                                    ...
                                    <div class="panel panel-default">
                                        ...
                                        <div class="panel-body">
                                            <table id="customers2" class="table datatable">
                                                ...
                                                <tbody>
                                                @php
                                                    $total = isset($total) ? $total + $renewals->amount : 0;
                                                @endphp
                                                @foreach ($renewal->ubpInspectionsList as $renewals)
                                                    <tr>
                                                        <td>{{ $renewals->business_name }}</td>
                                                        <td>{{ $renewals->bill }}</td>
                                                        <td>{{ date('d-m-Y', strtotime($renewals->date_applied)) }}</td>
                                                        <td>{{ $renewals->business_email }}</td>
                                                        <td>{{ $renewals->mobile_number }}</td>
                                                        <td>{{ $renewals->processing_status }}</td>
                                                        <td>{{ $renewals->payment_mode }}</td>
                                                        <td style="text-align: right">{{ number_format($renewals->amount,2) }}
                                                            @php($total += $renewals->amount)
                                                        </td>
                                                        <td>
                                                            {{ $renewals->processing_status }}
                                                        </td>
                                                    </tr>
                                                @endforeach
                                                </tbody>
